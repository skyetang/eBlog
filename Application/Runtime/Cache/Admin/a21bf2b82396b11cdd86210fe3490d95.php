...
<html>
	...
	<div class="main_content">
		<form  action="<?php echo U('Admin/Article/doUpdate');?>" onsubmit="return $.update(this)" method="post" name="articles">
		<div  class="article_content">
			<input type="text" value="<?php echo ($data["id"]); ?>" name="id" hidden="true"/>
			<label for="article_title">标题</label>
			<input type="text" id="article_title" name='title' value="<?php echo ($data["title"]); ?>" placeholder="未命名"/><br />
			<label for="article_cont">正文</label>
			<textarea id="edit_container" name="cont"><?php echo ($data["cont"]); ?></textarea>
			<div class="article_mianban">
				<label>分类</label>
	       	 	<select name="category">
	       	 	 <?php foreach($category as $key => $val): ?>
		       	 	<option <?php if($val['id'] == $data['cateid']):?>selected="selected"<?php endif;?> value="<?php echo ($val["id"]); ?>"><?php echo ($val["catename"]); ?></option>
		       	 	 <?php foreach($val['child'] as $k =>$v): ?>
		       	 		<option style="color: dimgray;" <?php if($v['id'] == $data['cateid']):?>selected="selected"<?php endif;?> value="<?php echo ($v["id"]); ?>">└─<?php echo ($v["catename"]); ?></option>
		       	 	<?php endforeach ?>
	       	 	<?php endforeach ?>
		       	</select><br />
		       	<label>作者</label>
		       	<select name="author">
		       	 	 <?php foreach($manager as $key => $vv): ?>
		       	 		<option <?php if($vv['username'] == $data['author']):?>selected="selected"<?php endif;?> value="<?php echo ($vv["username"]); ?>"><?php echo ($vv["username"]); ?></option>
		       	 	<?php endforeach ?>
		       	</select><br />
		       	 <label>时间</label>
		       	 <input type="text" id="datetimepicker" value="<?php echo ($data["time"]); ?>" name="time"><br />
		       	 <label>标签</label>
		       	 <input type="text" name="tags" value="<?php echo ($data["tags"]); ?>" />
